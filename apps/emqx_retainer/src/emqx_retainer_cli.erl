...
cmd(["info"]) ->
    emqx_ctl:print("retained/total: ~w~n", [emqx_retainer:get_table_size()]);

cmd(["topics"]) ->
    Topics = emqx_retainer:get_all_topics(),
    lists:foreach(fun(Topic) -> emqx_ctl:print("~s~n", [Topic]) end, Topics);

cmd(["clean"]) ->
    Size = emqx_retainer:clean(),
    emqx_ctl:print("Cleaned ~p retained messages~n", [Size]);

cmd(["clean", Topic]) ->
    Size = emqx_retainer:sync_delete(list_to_binary(Topic)),
    emqx_ctl:print("Cleaned ~p retained messages~n", [Size]);
